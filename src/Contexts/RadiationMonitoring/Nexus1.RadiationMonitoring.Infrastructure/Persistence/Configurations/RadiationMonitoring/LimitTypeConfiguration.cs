...
public sealed class LimitTypeConfiguration : IEntityTypeConfiguration<LimitType>
{
    public void Configure(EntityTypeBuilder<LimitType> builder)
    {
        builder.ToTable("LimitType", "RadiationMonitoring");
        builder.HasKey(x => x.Id).HasName("PK_RadiationMonitoring_LimitType");
        ...
        builder.HasIndex(x => x.Code).IsUnique().HasDatabaseName("UQ_RadiationMonitoring_LimitType_Code");
