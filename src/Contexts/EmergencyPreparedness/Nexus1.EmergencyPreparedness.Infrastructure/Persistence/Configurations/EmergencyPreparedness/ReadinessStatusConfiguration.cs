...
public sealed class ReadinessStatusConfiguration : IEntityTypeConfiguration<ReadinessStatus>
{
    public void Configure(EntityTypeBuilder<ReadinessStatus> builder)
    {
        builder.ToTable("ReadinessStatus", "EmergencyPreparedness");
        builder.HasKey(x => x.Id).HasName("PK_EmergencyPreparedness_ReadinessStatus");
        ...
        builder.HasIndex(x => x.Code).IsUnique().HasDatabaseName("UQ_EmergencyPreparedness_ReadinessStatus_Code");
