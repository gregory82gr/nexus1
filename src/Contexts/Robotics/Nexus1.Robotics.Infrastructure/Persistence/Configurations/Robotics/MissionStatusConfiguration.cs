...
public sealed class MissionStatusConfiguration : IEntityTypeConfiguration<MissionStatus>
{
    public void Configure(EntityTypeBuilder<MissionStatus> builder)
    {
        builder.ToTable("MissionStatus", "Robotics");
        builder.HasKey(x => x.Id).HasName("PK_Robotics_MissionStatus");
        ...
        builder.HasIndex(x => x.Code).IsUnique().HasDatabaseName("UQ_Robotics_MissionStatus_Code");
