...
public sealed class LinkEventToAlarmCommandHandlerTests : EventManagementComponentTestDatabase
{
    private static readonly DateTime NowUtc = new(2026, 8, 17, 12, 0, 0, DateTimeKind.Utc);

    private static LinkEventToAlarmCommandHandler CreateHandler(EventManagementDbContext dbContext) => new(
        new EfRepository<OperationalEvent, OperationalEventId>(dbContext),
        new EfRepository<EventAlarmLink, EventAlarmLinkId>(dbContext),
    ...
    [Fact]
    public async Task Links_the_event_to_the_seeded_alarm_event()
    {
        ...
        var result = await CreateHandler(dbContext).Handle(
            new LinkEventToAlarmCommand(operationalEventId, seed.AlarmEventId), CancellationToken.None);

        Assert.True(result.IsSuccess);

        await using var verifyContext = CreateDbContext();
        var stored = await verifyContext.EventAlarmLinks.FindAsync(new EventAlarmLinkId(result.Value));
        Assert.NotNull(stored);
        Assert.Equal(seed.AlarmEventId, stored!.AlarmEventId);
        Assert.Equal("SUPPORTING", stored.LinkRole);
    }
    ...
    public async Task Fails_when_the_referenced_operational_event_does_not_exist()
    {
        ...
        await using var dbContext = CreateDbContext();
        var result = await CreateHandler(dbContext).Handle(
            new LinkEventToAlarmCommand(OperationalEventId: 9999L, seed.AlarmEventId), CancellationToken.None);
