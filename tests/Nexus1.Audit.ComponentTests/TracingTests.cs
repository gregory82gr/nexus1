...
/// ActivityListener capture — mirrors Nexus1.RootCause.ComponentTests.
/// TracingTests (ADR-013 step 5).
/// </summary>
public sealed class TracingTests : AuditComponentTestDatabase
{
    ...
    private static List<CapturedSpan> CaptureSpans(Func<Task> scenario)
    {
        ...
        {
            ShouldListenTo = source => source.Name == NexusActivitySources.Audit,
    ...
    public void First_delivery_emits_a_committed_owner_span()
    {
        ...
        var span = Assert.Single(captured, s => s.Name == SpanNames.AuditEvidenceRecord);
        Assert.Equal(NexusActivitySources.Audit, span.SourceName);
