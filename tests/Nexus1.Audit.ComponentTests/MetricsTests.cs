...
/// <summary>Fast, deterministic proof of Audit's InboxOutcomes recording via in-process MeterListener capture (ch.52 52-AB) — mirrors Nexus1.RootCause.ComponentTests.MetricsTests.</summary>
public sealed class MetricsTests : AuditComponentTestDatabase
{
    ...
    public async Task First_delivery_records_a_committed_inbox_outcome()
    {
        ...
        var handler = new AuditVerdictMessageHandler(BuildScopeFactory(), metrics, NullLogger<AuditVerdictMessageHandler>.Instance);
        ...
        Assert.Equal(NexusActivitySources.Audit, measurement.Tags["nexus1.component"]);
    }
