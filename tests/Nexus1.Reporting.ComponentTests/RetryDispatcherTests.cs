...
/// <summary>Mirrors Nexus1.Audit/Compliance.ComponentTests.RetryDispatcherTests exactly — real LocalDB, no mocks for persistence.</summary>
public sealed class RetryDispatcherTests : ReportingComponentTestDatabase
{
    ...
    private async Task SeedDueTicketAsync(Guid messageId)
    {
        ...
        var ticket = new RetryTicket(
            Guid.NewGuid(), ReportingProjectionMessageHandler.ConsumerName, messageId, attempt: 1,
            RetryPolicies.ReportProject.PolicyId, "SimulatedTransientFailure", NowUtc, dueAtUtc: NowUtc,
