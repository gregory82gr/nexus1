...
/// <summary>Mirrors Nexus1.Audit.ComponentTests.RetryDispatcherTests exactly — real LocalDB, no mocks for persistence.</summary>
public sealed class RetryDispatcherTests : ComplianceComponentTestDatabase
{
    ...
    private async Task SeedDueTicketAsync(Guid messageId)
    {
        ...
        var ticket = new RetryTicket(
            Guid.NewGuid(), ComplianceVerdictMessageHandler.ConsumerName, messageId, attempt: 1,
            RetryPolicies.ComplianceRootCauseVerdicts.PolicyId, "SimulatedTransientFailure", NowUtc, dueAtUtc: NowUtc,
